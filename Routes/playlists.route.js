...
router.post("/addplaylist", verify, async (req, res) => {
  try {
    const newPlaylist = await playlistServices.addPlaylist(
      req.body,
      req.userName
    );
...
router.post("/addsong/:playlistid", verify, async (req, res) => {
  try {
    ...
    console.log("new favorite song");
    const newSong = await playlistServices.addSongToPlaylist(
      songData,
      playlistId
    );
    res.send(newSong);
  } catch (err) {
    console.log("new favorite song error");
    res.status(400).send(err);
  }
});
...
router.get("/likedsongs", verify, async (req, res) => {
  try {
    const likesSongsPlaylist = await playlistServices.getLikedSongsPlaylist(
      req.userName
    );
...
router.post("/deletesong/:playlistid", verify, async (req, res) => {
  try {
    ...
    const updatedPlaylist = await playlistServices.deleteSongFromPlaylist(
      playlistId,
      songId
    );
